...
class ConstraintChecker {
    ...
    checkException(constraint, context) {
        if (!constraint.exceptions)
            return false;
        return constraint.exceptions.some((ex) => context[EXCEPTION_FIELD_MAP[ex]] === true);
    }
    /**
     * 根据约束层级获取严重性
     */
     ...
     * 判断约束是否匹配当前触发条件
     */
    matchesTrigger(constraint, operation) {
        const triggers = (0, exec_1.normalizeTriggers)(constraint.trigger);
        return triggers.includes(operation);
    }
    /**
     * 记录约束检查的 trace
     */
    recordTrace(collector, constraint, checkResult, context) {
        collector.record({
     ...
     * 查找 src/ 目录中的源文件
     */
    findSourceFiles(dir, projectPath) {
        const results = [];
        let entries;
        try {
...
async function checkConstraint(constraintId, context) {
    ...
    const constraint = constraints.ironLaws[constraintId] ||
